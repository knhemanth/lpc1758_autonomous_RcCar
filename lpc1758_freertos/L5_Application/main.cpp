...
#define can_mssg_id 0x024
#define can_baud_kbps 100


SemaphoreHandle_t binary_sem = 0;
can_msg_t tx_mssg;
extern char rec_str[bt_data_len];
bool can_init_stat = 0;
/**
...
void can_init()
{
    ...
    tx_mssg.frame_fields.data_len = bt_data_len;
...
class bt_uart_task : public scheduler_task
{
    public:
        ...
        bool run(void *p)
        {
           ...
           {
               tx_mssg.data.bytes[i] = (uint8_t)(*(bt_str + i));
               tx_mssg.data.bytes[i] -= 48;
               printf("\nval at %d = %d",i,tx_mssg.data.bytes[i]);
           }
